...
function MyLists() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log("List IDs", currentIDs);
      let list = [];
      for (let id of currentIDs) {
        console.log("for id", id);
        let listDoc = await db
        ...
        list.push({ id, list: listDoc.data() });
      }
      setCurrenLists(list);
    };
            ...
            >
              ...
              <ListItemSecondaryAction>
                <IconButton>
                  <DeleteForever
                    onClick={() => {
                      console.log("remove anime", item);
                      db.collection("animeLists").doc(item.id).delete();
                      db.collection("users").doc(userDocID).update({
                        myAnimeLists: firebase.firestore.FieldValue.arrayRemove(
                          item.id
                        ),
                      });
                      setUpdate(!update);
                      ...
                      <IconButton
                        onClick={() => {
                          ...
                          db.collection("animeLists")
                            .doc(item.id)
                            .update({
                              contents: firebase.firestore.FieldValue.arrayRemove(
                                element
                              ),
                            });
                          setUpdate(!update);
